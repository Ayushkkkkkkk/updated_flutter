...
class _TaskListScreenState extends State<TaskListScreen> {
  ...
  void _scheduleTasks() {
    ...
    DateTime startTimeRange =
        _departureTime.subtract(Duration(minutes: totalTimeRequired));
  ...
  switch (priority) {
    ...
    default:
      return Color.fromARGB(
          255, 241, 241, 241); // Default color if priority is not recognized
  }
}
...
class TaskCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    String endTime = DateFormat('hh:mm:ss a').format(
      DateTime(2024, 3, 19, (task.startTime + task.timeTaken) ~/ 60,
          (task.startTime + task.timeTaken) % 60),
    );

    // Calculate scheduled remainder
    int scheduledRemainder =
        task.startTime - departureTime.hour * 60 - departureTime.minute;
